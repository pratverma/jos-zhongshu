...
boot_alloc(uint32_t n, uint32_t align)
{
	...
	boot_freemem = v + n;
	if((uintptr_t)boot_freemem >= 0xFFFFFFFF)
	{
		panic("Ouf of memory!!\n");
		return NULL;
	}
...
i386_vm_init(void)
{
	...
	// segmentation.
	// We have already finish the mapping of KERNBASE in boot_map_segment

	// Map VA 0:4MB same as VA KERNBASE, i.e. to PA 0:4MB.
	// (Limits our kernel to <4MB)
	// When first turning on paging and not turning off segmentation
	// There is mapping for KERNBASE in pd but no mapping for 0 in pd.
	// So we copy the mapping of KERNBASE to 0. 
	...
	// we load the new gdt and make it has the same effect of turning off segmentation
	// At this time, pgdir[0] is no longer needed and should be cleared.
...
page_init(void)
{
	...
	for (i = 0; i < npage; i++) {
		...
		if(i == 0)
			pages[i].pp_ref = 1;
		//IO hole
		else if(i >= (physaddr_t)(IOPHYSMEM / PGSIZE) 
				&& i < (physaddr_t)(EXTPHYSMEM / PGSIZE))
			pages[i].pp_ref = 1;
		//JOS kernel,page directories and other data strutures allocated by boot_alloc
		else if((i >= (physaddr_t)(EXTPHYSMEM / PGSIZE)) 
				&& (i< (physaddr_t)((EXTPHYSMEM + (uintptr_t)boot_freemem - KERNBASE) / PGSIZE)))
			pages[i].pp_ref = 1;
		else 
		{
			pages[i].pp_ref = 0;
...
page_alloc(struct Page **pp_store)
{
	...
	if((*pp_store = LIST_FIRST(&page_free_list)) != 0)
	{	
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	struct Page *pt = NULL;
	pte_t *p = NULL;
	physaddr_t ptaddr = pgdir[PDX(va)];
	int perm;
	if(!(ptaddr & PTE_P))
	{
		if(create == 0)
		...
		{
			if(page_alloc(&pt) == -E_NO_MEM)
				return NULL;
			else
			{
				memset(page2kva(pt), 0x0, PGSIZE);
				pt->pp_ref = 1;
				ptaddr = PTE_ADDR(page2pa(pt));
				...
				pgdir[PDX(va)] = ptaddr|perm;
				p = KADDR(ptaddr);
				return &p[PTX(va)];
			}
		}
	}
	p = KADDR(PTE_ADDR(ptaddr));
	return &p[PTX(va)];
	/*
	...
	if (!(*pgdir & PTE_P))
	{
		...
		else
		{
			...
			else
			{
				memset(page2kva(pt), 0x0, PGSIZE);
				pt->pp_ref = 1;
				*pgdir = PTE_ADDR(page2pa(pt))|PTE_U|PTE_W|PTE_P;
				p = (pte_t*)KADDR(PTE_ADDR(page2pa(pt)));
				return &p[PTX(va)];
			}			
		}	
	}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	struct Page *page_dec = NULL;
	page_dec= page_lookup(pgdir,va,&pte_store);
	if(page_dec)
	{
		page_decref(page_dec);
