...
piperead(struct Fd *fd, void *vbuf, size_t n, off_t offset)
{
	...
	char *buf = vbuf;
	//cprintf("read:piperead rpos:%08x,wpos:%08x\n",p->p_rpos, p->p_wpos);
	...
	while((p->p_rpos < p->p_wpos) && count < n)	
	{
		buf[count] = p->p_buf[p->p_rpos % PIPEBUFSIZ];
		count ++;
		p->p_rpos ++;
		//cprintf("piperead rpos:%08x,wpos:%08x\n",p->p_rpos, p->p_wpos);
...
pipewrite(struct Fd *fd, const void *vbuf, size_t n, off_t offset)
{
	...
	struct Pipe *p = (struct Pipe*) fd2data(fd);
	size_t count = 0;
	//cprintf("write:pipewrite rpos:%08x,wpos:%08x\n",p->p_rpos, p->p_wpos);

	while((p->p_wpos - p->p_rpos) >= PIPEBUFSIZ)
	{
		//cprintf("in while:pipewrite rpos:%08x,wpos:%08x\n",p->p_rpos, p->p_wpos);
	...
	while(count < n)
	{
		p->p_buf[p->p_wpos % PIPEBUFSIZ] = ((char*)vbuf)[count];
		count ++;
		p->p_wpos ++;
		//cprintf("pipewrite rpos:%08x,wpos:%08x\n",p->p_rpos, p->p_wpos);
		while((p->p_wpos - p->p_rpos) >= PIPEBUFSIZ && count < n)
		{
			if(_pipeisclosed(fd,p))
				return count;
			sys_yield();
