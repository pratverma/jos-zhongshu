...
fd_alloc(struct Fd **fd_store)
{
	...
	{
		//cprintf("i:%08x\n",i);
		va = (uintptr_t)INDEX2FD(i);
		if(!((vpd[PDX(va)] & PTE_P) && (vpt[VPN(va)] & PTE_P)))
		{
			*fd_store = (struct Fd *)va;
			return 0;
		}
	}
	*fd_store = 0;
...
fd_lookup(int fdnum, struct Fd **fd_store)
{
	// LAB 5: Your code here.
	uintptr_t va;
	if(fdnum < MAXFD && fdnum >= 0)
	{
		va = (uintptr_t)INDEX2FD(fdnum);
		if((vpd[PDX(va)] & PTE_P) && (vpt[VPN(va)] & PTE_P))
		{
...
fd_close(struct Fd *fd, bool must_exist)
{
	...
	// This is where we need to do to keep it consitent
...
dup(int oldfdnum, int newfdnum)
{
	...
	if ((r = fd_lookup(oldfdnum, &oldfd)) < 0)
		return r;
	//cprintf("dup close newfd:%08x\n",newfdnum);
	close(newfdnum);
		
	//cprintf("dup close return r:%08x\n",r);
