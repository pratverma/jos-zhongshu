	...
	for (i = 0; i < MAXOPEN; i++) {
		...
		case 1:
			...
			*o = &opentab[i];
			...
			return (*o)->o_fileid;
			//So origin reference will always be 1
		}
	}
	return -E_MAX_OPEN;
}
...
openfile_lookup(envid_t envid, uint32_t fileid, struct OpenFile **po)
{
	...
	//if pageref is 1, it means there is not any mapping yet
...
serve_close(envid_t envid, struct Fsreq_close *rq)
{
	...
	if((r = openfile_lookup(envid, rq->req_fileid, &o)) < 0)
		goto out;
	file_close(o->o_file);
	//No need to unmap fd in this way like below as client will be responsible to unmap it.
	/*
	if (pageref(o->o_fd) == 2) {
